...
fig, ax = plt.subplots(5, 2, figsize=(7, 9), sharex=True)
...
for i in range(5):
    ...
    kick_wait_time = get_parameter("20221205", shot, "kick_wait_time")
    ax[i][0].text(0.01, 0.1, "Wait time = {:} ms\nTOF = 25 ms".format(kick_wait_time), horizontalalignment='left', verticalalignment='center', transform=ax[i][0].transAxes)
...
for i in range(5):
    ...
    kick_wait_time = get_parameter("20221205", shot, "kick_wait_time")
    ax[i][1].text(0.01, 0.1, "Wait time = {:} ms\nTOF = 25 ms".format(kick_wait_time), horizontalalignment='left', verticalalignment='center', transform=ax[i][1].transAxes)
