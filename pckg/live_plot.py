    ...
    if os.path.exists(path):
        ...
        if len(dirList) > 0:
            ...
            for run in output:
                ...
                png_files = glob.glob(run_path + "/*.png")
                if png_files:
                    # print("Yes, found .png files in:", run)
                    return datestring, run
        else:
            # If there are no directories in the specified path
...
def get_fit_parameter(date, seq, param):
    ...
    condensed = False
    var = 0
    if os.path.exists(path + date + '/' + run_id + f'/fit_bi_{run_id}.png'):
        with open(path + date + '/' + run_id + '/fit_bimodal.param') as paramfile:
            csvreader = csv.reader(paramfile, delimiter=',')
            next(csvreader)
            next(csvreader)
    ...
    elif os.path.exists(path + date + '/' + run_id + f'/fit_g_{run_id}.png'):
    ...
    elif os.path.exists(path + date + '/' + run_id + f'/fit_tf_{run_id}.png'):
    ...
    elif os.path.exists(path + date + '/' + run_id + f'/viewer__{run_id}.png'):
        var = 0
