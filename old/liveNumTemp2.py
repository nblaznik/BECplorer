...
xs, ys = updateData(init, last, datestring, "ntotal")
...
ylim = max(-0.05 * max(ys_tz), min(ys_tx) - ((max(ys_tz) - min(ys_tx)) * edgey / (1 - 2 * edgey))), max(ys_tz) + ((max(ys_tz) - min(ys_tx)) * edgey / (1 - 2 * edgey))
ax2x.set_xlim(xlim)
ax2x.set_ylim(ylim)
