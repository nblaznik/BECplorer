...
def updateData(init, last, datestring):
...
xs, ys = updateData(init, last, datestring)
...
ax.set_title("Particle number through the runs.\n", fontsize=15, weight='bold')
ax.set_xlabel("RUN")
...
ylim = max(0, min(ys_low) - ((max(ys_high) - min(ys_low)) * edgey / (1 - 2 * edgey))), max(ys_high) + ((max(ys_high) - min(ys_low)) * edgey / (1 - 2 * edgey))
ax.set_xlim(xlim)
ax.set_ylim(ylim)
